...
# 1. Insert LoginPage before App
$appPattern = '(?s)\s+const App = \(\) => \{'
$loginPageCode = @"
...
$content = [regex]::Replace($content, $appPattern, $loginPageCode + [System.Environment]::NewLine + "        const App = () => {", 1)

# 2. Add State in App
$statePattern = 'const App = \(\) => \{(.*?const \[page, setPage\] = useState\(''Planificacion''\);)'
$stateReplacement = "const App = () => {`r`n            const [isAuthenticated, setIsAuthenticated] = React.useState(false);`r`n            const [currentUser, setCurrentUser] = React.useState(null);`r`n            `$1"
$content = [regex]::Replace($content, $statePattern, $stateReplacement, 1)

# 3. Add Login check in App return
$returnPattern = '(?s)(const App = \(\) => \{.*?)(return \(\s+<div class="flex min-h-screen">)'
$returnFixed = @"
`$1            if (!isAuthenticated) {
            ...
            `$2
"@
$content = [regex]::Replace($content, $returnPattern, $returnFixed, 1)

# 4. Sidebar changes
$sidebarPattern = '<div class="p-6 border-t border-slate-100 mt-auto">'
$newSidebar = @"
...
# Remove the old user block
$oldBlockPattern = '(?s)<div class="flex items-center gap-3 mb-2">.*?Usuario Demo.*?PRO</span>\s*</div>'
$content = [regex]::Replace($content, $oldBlockPattern, "")

[System.IO.File]::WriteAllText($path, $content)
Write-Output "Applied fixes with REGEX V2 successfully."
