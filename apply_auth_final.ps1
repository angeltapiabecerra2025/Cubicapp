...
# Inject LoginPage before line 2891 (index 2890)
$loginPage = @"
...
# Update App component lines
# Add authentication state to App
$lines[2890] = $loginPage + "`r`n`r`n        const App = () => {"
$lines[2891] = "            const [isAuthenticated, setIsAuthenticated] = React.useState(false);"
$lines[2891] += "`r`n            const [currentUser, setCurrentUser] = React.useState(null);"
$lines[2891] += "`r`n" + $lines[2891] # Keep the original page state line? No, let's be precise.
$lines[2891] = "            const [isAuthenticated, setIsAuthenticated] = React.useState(false);"
$lines[2892] = "            const [currentUser, setCurrentUser] = React.useState(null);"
# Shift original lines? No, let's just use replacement logic.

# Actually, the safest way is to find the line by its EXACT content in the array
$appIdx = -1
for ($i = 0; $i -lt $lines.Count; $i++) {
    if ($lines[$i] -match "^\s*const App = \(\) => \{$") {
        $appIdx = $i
        break
    }
}
...
if ($appIdx -gt -1) {
    # Insert LoginPage BEFORE App
    $newLines = @()
    for ($i = 0; $i -lt $appIdx; $i++) { $newLines += $lines[$i] }
    $newLines += $loginPage
    $newLines += ""
    $newLines += $lines[$appIdx]
    # Add auth state
    $newLines += "            const [isAuthenticated, setIsAuthenticated] = React.useState(false);"
    $newLines += "            const [currentUser, setCurrentUser] = React.useState(null);"
    for ($i = $appIdx + 1; $i -lt $lines.Count; $i++) { $newLines += $lines[$i] }
    $lines = $newLines
}

# Add conditional return
$returnIdx = -1
# Search AFTER App definition
$startSearch = 0
for ($i = 0; $i -lt $lines.Count; $i++) {
    if ($lines[$i] -match "^\s*const App = \(\) => \{$") { $startSearch = $i; break }
}
...
if ($returnIdx -gt -1) {
    $newLines = @()
    for ($i = 0; $i -lt $returnIdx; $i++) { $newLines += $lines[$i] }
    $newLines += "            if (!isAuthenticated) {"
    $newLines += "                return <LoginPage onLogin={(user) => { "
    $newLines += "                    setIsAuthenticated(true); "
    $newLines += "                    setCurrentUser(user); "
    $newLines += "                }} />;"
    $newLines += "            }"
    $newLines += ""
    for ($i = $returnIdx; $i -lt $lines.Count; $i++) { $newLines += $lines[$i] }
    $lines = $newLines
}
...
$sidebarIdx = -1
for ($i = 0; $i -lt $lines.Count; $i++) {
    if ($lines[$i] -match '<div class="p-6 border-t border-slate-100 mt-auto">') {
        $sidebarIdx = $i
        break
    }
}
...
"@
    # Remove old block (roughly 14 lines)
    $lines[$sidebarIdx] = $newSidebar
    for ($j = 1; $j -le 13; $j++) { $lines[$sidebarIdx + $j] = "" }
}

[System.IO.File]::WriteAllLines($path, $lines, [System.Text.Encoding]::UTF8)
Write-Output "Applied successfully."
