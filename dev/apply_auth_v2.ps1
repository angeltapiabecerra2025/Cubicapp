...
# Avoid BOM issues by using explicit UTF8 encoding without BOM if possible, 
# but ReadAllText with UTF8 usually handles it fine.
...
$oldAppStart = '        const App = () => {'
if ($content.Contains($oldAppStart)) {
    $content = $content.Replace($oldAppStart, $loginPage + [System.Environment]::NewLine + $oldAppStart)
}

$oldReturn = '            return ('
# Match only the one after App
$appPos = $content.IndexOf("const App = () => {")
$returnPos = $content.IndexOf($oldReturn, $appPos)
if ($returnPos -gt -1) {
    $injReturn = @"
            if (!isAuthenticated) {
...
"@
    $content = $content.Insert($returnPos, $injReturn)
}

$sidebarSearch = '<div class="p-6 border-t border-slate-100 mt-auto">'
$newSidebar = @"
...
# Find the specific sidebar div and replace it
# To avoid replacing multiple divs, we use a more unique pattern if possible
$oldSidebarBlockPattern = '(?s)<div class="p-6 border-t border-slate-100 mt-auto">.*?Usuario Demo.*?PRO</span>\s*</div>\s*</div>'
# Actually let's just use simple Replace for the start div and then remove the duplicate part
$content = $content.Replace($sidebarSearch, $newSidebar)

# Remove the duplicate part that was pushed down
$duplicatePart = @"
...
"@
$content = $content.Replace($duplicatePart, "")
